...
function convertProgramIdPermissions(
    permissions: Partial<Record<WalletAdapterNetwork, string[]>>
): Partial<Record<Network, string[]>> {
    const networkMapping: Record<WalletAdapterNetwork, Network> = {
        [WalletAdapterNetwork.MainnetBeta]: Network.AleoMainnet,
        [WalletAdapterNetwork.TestnetBeta]: Network.AleoTestnet,
        [WalletAdapterNetwork.Testnet]: Network.AleoTestnet,
    };

    const convertedPermissions: Partial<Record<Network, string[]>> = {};

    for (const [key, value] of Object.entries(permissions)) {
        const network = networkMapping[key as WalletAdapterNetwork];
        if (network) {
            convertedPermissions[network] = value;
        }
    }
...
export class PuzzleWalletAdapter extends BaseMessageSignerWalletAdapter {
    ...
    private _appIconUrl: string | undefined;
    private _appDescription: string | undefined;
    private _programIdPermissions: ProgramIdPermissions;
    ...
    async requestTransaction(transaction: AleoTransaction): Promise<string> {
        try {
            ...
            try {
                ...
                } else if (result.eventId === undefined || result.eventId === '') {
    ...
    async connect(
        ...
        try {
            ...
            try {
                ...
                this._network = [WalletAdapterNetwork.Testnet, WalletAdapterNetwork.TestnetBeta].includes(network) ? Network.AleoTestnet : Network.AleoMainnet;
